//+------------------------------------------------------------------+
//|                                          IAX_v6_GODMODE.mq5       |
//|      Institutional Adaptive XAUUSD Trading System  —  v6.0        |
//|      "GODMODE" — probabilistic multi-factor decision engine,      |
//|      layered capital governors, online adaptive learning.         |
//|                                                                    |
...
//|  is logged. Read docs/IAX_v6_TECHNICAL.md before trading live.     |
//+------------------------------------------------------------------+
#property copyright "IAX v6.0 GODMODE"
#property link      ""
#property version   "6.00"
#property description "Institutional Adaptive XAUUSD EA — probabilistic 9-factor decision engine with layered governors and online adaptive learning. No profit is promised."
...
input long   InpMagic                 = 660001; // Base magic number (scalp). Swing = Magic+7
...
void BuildFingerprint()
  {
   g_buildFingerprint = StringFormat(
      "IAX6-%s-%s-Profile%d-Stage%d-Sig%s-Trend%s-Magic%d-%s",
...
string GVName(string key)
  {
   return StringFormat("IAX6_%s_%d_%s", g_symbol, (int)InpMagic, key);
  }
...
int OnInit()
  {
   ...
   g_stateFileName   = StringFormat("IAX6_state_%s_%d.csv", g_symbol, (int)InpMagic);
   g_journalFileName = StringFormat("IAX6_journal_%s_%d.csv", g_symbol, (int)InpMagic);
   LoadPersistedState();
   ...
   if(InpShowKillSwitch)
     {
      ObjectCreate(0, "IAX6_KillSwitch", OBJ_BUTTON, 0, 0, 0);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_XDISTANCE, 10);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_YDISTANCE, 10);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_XSIZE, 140);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_YSIZE, 28);
      ObjectSetString(0, "IAX6_KillSwitch", OBJPROP_TEXT, "FLATTEN ALL");
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_BGCOLOR, clrFireBrick);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_COLOR, clrWhite);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_SELECTABLE, false);
     }
...
void OnDeinit(const int reason)
  {
   ...
   ObjectDelete(0, "IAX6_KillSwitch");
   Comment("");
   PrintEndOfRunReport();
...
void OnTick()
  {
   ...
   if(g_killSwitchEngaged)
      return; // kill switch latched: no new decisions, management loop below still runs to close out

   // Position management runs every tick regardless of new-bar status.
   ...
   bool newSignalBar = IsNewBar(g_signalTF, g_lastSignalBarTime);
   if(!newSignalBar)
     {
      if(InpShowDashboard) UpdateDashboard();
      return;
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
   if(id == CHARTEVENT_OBJECT_CLICK && sparam == "IAX6_KillSwitch")
     {
      g_killSwitchEngaged = true;
      FlattenAll("Manual kill switch engaged.");
      ObjectSetInteger(0, "IAX6_KillSwitch", OBJPROP_STATE, false);
      if(InpPushAlertsOn) SendNotification("IAX6 " + g_symbol + ": KILL SWITCH engaged, all positions flattened.");
     }
  }
...
//====================================================================
// [RunDecisionEngine] — the heart of the system, called once per new
// signal-TF bar. Never predicts the next candle; estimates who currently
// has the advantage and gates execution on confidence.
//====================================================================
void RunDecisionEngine()
  {
   ...
   // Probabilities normalised over the ACTIVE weight pool only, so a
   // "lite" mode (some factors disabled/unavailable) still reaches the gate.
   ...
   if(activeWeightSum <= 0.0)
     {
      g_buyProb = g_sellProb = 0.0; g_confidence = 0.0;
      Log("DECISION: no active factor weight pool — skipping bar.");
      return;
     }
   ...
   double edge = MathAbs(g_buyProb - g_sellProb);
   g_confidence = edge * g_regimeQuality * g_sessionQuality * 100.0;
   ...
   // agrees with the still-running trend direction get dampened).
   ...
   if(InpVerboseDecisionLog)
      Log(StringFormat("BAR %s | Regime=%s(qx%.2f) Session=%s(qx%.2f) buyP=%.3f sellP=%.3f edge=%.3f conf=%.1f ignite=%s exh=%s dir=%d",
          TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES), EnumToString(g_regime), g_regimeQuality,
          EnumToString(g_session), g_sessionQuality, g_buyProb, g_sellProb, edge, g_confidence,
          g_ignitionBar?"Y":"N", g_exhaustionBar?"Y":"N", direction));

   if(direction == 0)
     { LogBlock(BLOCK_LOW_CONFIDENCE, "no directional edge this bar"); return; }

   if(edge < InpMinVoteEdge)
     { LogBlock(BLOCK_VOTE_EDGE_TOO_THIN, StringFormat("edge %.3f < min %.3f", edge, InpMinVoteEdge)); return; }

   if(g_confidence < eff_MinConfidenceToTrade)
     {
      LogBlock(BLOCK_LOW_CONFIDENCE, StringFormat("confidence %.1f < %.1f", g_confidence, eff_MinConfidenceToTrade));
      return;
     }

   // Confidence gate passed — this is a "shadow gate" candidate even if a
   // downstream governor blocks it, tracked separately for the end-of-run report.
   ...
   ExecuteEntry(direction, g_confidence);
  }
   ...
   for(int i = 0; i < InpEntriesPerSignal; i++)
     {
      ...
      if(InpPushAlertsOn)
         SendNotification(StringFormat("IAX6 %s: %s entry lot=%.2f conf=%.1f", g_symbol, direction==1?"BUY":"SELL", lot, confidence));
     }
