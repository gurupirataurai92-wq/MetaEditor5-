...
//|  Single-file build of V75EA v5.00 for MetaEditor 5: paste into   |
//|  MQL5/Experts, press F7 — no Include-folder setup required.      |
//|                                                                  |
//|  Grounded in the V75 arithmetic (driftless GBM, 75% annual vol). |
//|  The EdgeModel computes the LIVE expected value of every trade   |
//|  from the two-barrier hitting probability with measured drift    |
//|  (gambler's-ruin formula), net of spread — and refuses trades    |
//|  with no measurable edge. Under pure randomness EV = -spread for |
//|  every geometry; profits can only come from real deviations      |
//|  (persistence/clustering), which this EA measures, never assumes.|
//|                                                                  |
//|  Godmode traits: on-chart dashboard + PAUSE/CLOSE-ALL, bounded   |
//|  opt-in recovery, session filter, daily profit target, push      |
//|  notifications — all under hard risk rails.                     |
//+------------------------------------------------------------------+
